...
function Navbar() {
  ...
  const { data: notificatiions } = useQuery({
    queryKey: ["notificatiions"],
    queryFn: async () => {
      const res = await axiosInstance.get("/notifications/");
      return res.data;
    },
    enabled:
      !!authUser /* bang bang (operator) return true or false if we have auth user , here if auth user = true, only then go & fetch this queryFn*/,
  });
  ...
  const unreadNotificationCount = notificatiions?.data?.filter(
